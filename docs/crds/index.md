...
All 12 kinds live in the API group `polaris.k8s.calific.io/v1alpha1`, are namespaced, and share the `polaris` kubectl category (`kubectl get polaris -A` lists all of them). Every kind has a `Ready` condition, a `Synced` condition, `.status.observedGeneration`, and — except `PolarisConnection`, which owns no remote state — a `polaris.k8s.calific.io/finalizer` that deletes the Polaris-side object before the Kubernetes record disappears.

## Naming

A resource's name in Polaris defaults to `.metadata.name`. Every kind accepts an optional `spec.name` to override that — useful when the Polaris-side name needs characters Kubernetes object names don't allow (Kubernetes names are RFC 1123 subdomains: lowercase alphanumerics and `-` only; Polaris names additionally allow `_`).
...
Every `*Ref` field (`connectionRef`, `catalogRef`, `namespaceRef`, `principalRef`, `principalRoleRef`, `catalogRoleRef`, `tableRef`, `viewRef`, …) has the same shape:
...
This project deliberately doesn't use `corev1.LocalObjectReference`/`corev1.ObjectReference` — purpose-built `*Ref` structs keep each reference's cross-namespace behavior explicit and independently documented.

## Kinds at a glance

| Kind | Short name | Parent(s) | Purpose |
|---|---|---|---|
| [`PolarisConnection`](connection.md) | `pconn` | — | Server URL + OAuth credentials. Everything else attaches here, directly or via a parent. |
| [`PolarisCatalog`](catalog.md) | `pcat` | Connection | A top-level Iceberg catalog and its storage config. |
| [`PolarisNamespace`](namespace.md) | `pns` | Catalog (+ optional parent Namespace) | A logical container for tables/views; nestable. |
| [`PolarisTable`](table.md) | `ptbl` | Namespace | An Iceberg table: schema, partition spec, write format. |
| [`PolarisView`](view.md) | `pview` | Namespace | An Iceberg view: schema + SQL definition. |
| [`PolarisPolicy`](policy.md) | `ppol` | Namespace | A typed policy (e.g. compaction) attached to a namespace. |
...
| [`PolarisGrant`](grant.md) | `pgrant` | CatalogRole (+ target) | Attaches one privilege on one target to a catalog role. |
