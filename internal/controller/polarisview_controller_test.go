...
// These specs drive PolarisViewReconciler against the real envtest API
// server, mirroring TestPolarisViewReconcile_CreatesWhenMissing in
...
var _ = Describe("PolarisView Controller", func() {
	const testNamespace = "polarisview-test"
	...
	Context("When reconciling a view that doesn't exist yet Polaris-side", func() {
		const resourceName = "orders-v"
		nn := types.NamespacedName{Name: resourceName, Namespace: testNamespace}

		It("creates the view and reaches Ready=True", func() {
			_, catName, nsName := seedParents("")

			By("creating the PolarisView CR")
			view := &polarisv1alpha1.PolarisView{
				...
				Spec: polarisv1alpha1.PolarisViewSpec{
					...
					Schema: polarisv1alpha1.IcebergSchema{
						Fields: []polarisv1alpha1.IcebergField{{ID: 1, Name: "id", Type: "long"}},
					},
			...
			base := "/api/catalog/v1/" + catName + "/namespaces/" + nsName + "/views"
			fp := newFakePolaris(GinkgoT())
			fp.reply("GET", base+"/"+resourceName, http.StatusNotFound, nil)
			fp.reply("POST", base, http.StatusOK, map[string]any{
				"metadata-location": "s3://bucket/orders-v/metadata.json",
				"metadata": map[string]any{
					"view-uuid":          "uuid-orders-v",
					"format-version":     1,
					"location":           "s3://bucket/orders-v",
			...
			got := &polarisv1alpha1.PolarisView{}
	...
	Context("When deleting a view", func() {
		...
		It("drops it Polaris-side and removes the finalizer", func() {
			...
			fp.reply("POST", base, http.StatusOK, map[string]any{
				...
				"metadata": map[string]any{
					"view-uuid": "uuid", "format-version": 1, "location": "s3://bucket/to-delete",
					"current-version-id": 1, "schemas": []any{}, "versions": []any{}, "version-log": []any{},
				},
			})
